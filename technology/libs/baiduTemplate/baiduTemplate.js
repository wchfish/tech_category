 ...
 * http://baidufe.github.com/BaiduTemplate 开源协议：BSD License 浏览器环境占用命名空间
 * baidu.template ，nodejs环境直接安装 npm install baidutemplate
 * 
 * @param str{String}
 *            dom结点ID，或者模板string
 * @param data{Object}
 *            需要渲染的json对象，可以为空。当data为{}时，仍然返回html。
...
(function(window) {
	...
	var baidu = typeof module === 'undefined' ? (window.baidu = window.baidu
			|| {}) : module.exports;
	...
	bt._analysisStr = function(str) {
		...
		str = str
				...
				// 对变量后面的分号做容错(包括转义模式 如<%:h=value%>) <%=value;%> 排除掉函数的情况
				// <%fun1();%> 排除定义变量情况 <%var val='test';%>
				.replace(
						new RegExp("(" + _left + ":?[hvu]?[\\s]*?=[\\s]*?[^;|"
										+ _right + "]*?);[\\s]*?" + _right, "g"),
						"$1" + _right_)
		...
		str = str
				...
				// 会被替换为
				// if(list.length=5){_template_fun_array.push('<h2>',list[4],'</h2>');}
